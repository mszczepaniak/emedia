...
namespace EmediaQuest1
{
    ...
    /// </summary>
    struct COMPLEX // liczby zespolone
    {
        ...
        public float Magnitude() // amplituda, policzenie
        {
            return ((float)Math.Sqrt(real * real + imag * imag)); // pierwiastek z sumy kwadratow czesci rzeczywistej i czesci urojonej
        }
        public float Phase() // faza, policzenie
        {            
             return ((float)Math.Atan(imag / real)); // funkcja Atan, co to jest? odwrotny tangens? DO SPRAWDZENIA !!!
        }
    }
    ...
    class FFT
    {
        ...
        int Width, Height; // wysokosc i szerokosc obrazka
